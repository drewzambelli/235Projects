...
//PROJECT 6 TESTING
    ...
    Post* ptrPost2 = new General("ptrPost2 Body Title", "ptrPost2", "Bruce");
    Post* ptrPost3 = new General("ptrPost3 Title", "ptrPost3", "Neil");
    Post* ptrPost4 = new General("ptrPost4 Title", "ptrPost4 Body", "Mungo");
    Post* ptrPost5 = new General("ptrPost5 Title", "ptrPost5 Body", "Bob");
    Post* ptrPost6 = new General("ptrPost6 Title", "ptrPost6 Body", "John");
    Post* ptrPost7 = new General("ptrPost7 Title", "ptrPost7 Body", "Chuck");

    Account* drew1 = new Account("Billy", "myPassword");
    ...
    cout << "Size of network: " << network1->getSizeOfNetwork() << endl;

    ptrPost1->setPriority(Post::LOW);
    ptrPost2->setPriority(Post::LOW);
    ptrPost3->setPriority(Post::LOW);
    ptrPost4->setPriority(Post::LOW);
    ptrPost5->setPriority(Post::HIGH);
    ptrPost6->setPriority(Post::LOW);
    ptrPost7->setPriority(Post::MEDIUM);
    ...
    drew1->addPost(ptrPost2);
    drew1->addPost(ptrPost3);
    drew1->addPost(ptrPost4);
    drew1->addPost(ptrPost5);
    drew1->addPost(ptrPost6);
    drew1->addPost(ptrPost7);

    cout << "get min likes: ";
    cout << drew1->getMinLikes()->getLikes() << " " << drew1->getMinLikes()->getUsername() << endl;
    cout << "get max likes: ";
    cout << drew1->getMaxLikes()->getLikes() << " " << drew1->getMaxLikes()->getUsername() << endl;

    // for (int i = 0; i <drew1->postLikes_.size(); i++){
    //     cout << drew1->postLikes_[i]->getLikes() << " " << drew1->postLikes_[i]->getTitle() << endl;
    // }

    vector<Post*> newVec;
    newVec = drew1->increasingPriorityOldestToNewest();
    cout << "Testing oldest to newest with increasing priority" << endl;
    for(int i = 0; i<newVec.size(); i++) {
        cout << newVec[i]->getPriority() << " and i index: " << i << " and " << newVec[i]->getUsername() << endl;
    }

    cout << "Testing newest to oldest with increasing priority" << endl;
    vector<Post*> nextVec = drew1->increasingPriorityNewestToOldest();


    for(int i = 0; i<nextVec.size(); i++) {
        cout << nextVec[i]->getPriority() << " and i index: " << i << " and " << nextVec[i]->getUsername() << endl;
    }

    cout << "Decreasing Priority / OLDEST TO NEWEST" << endl;
    vector<Post*> anotherVec = drew1->decreasingPriorityNewestToOldest();
    for(int i = 0; i<anotherVec.size(); i++) {
        cout << anotherVec[i]->getPriority() << " and i index: " << i << " and " << anotherVec[i]->getUsername() << endl;
    }

    cout << "Decreasing Priority / NEWEST TO OLDEST" << endl;
    vector<Post*> revVec = drew1->decreasingPriorityNewestToOldest();

    for(int i = 0; i<revVec.size(); i++) {
        cout << revVec[i]->getPriority() << " and i index: " << i << " and " << revVec[i]->getUsername() << endl;
    }

    drew1->updatePost(ptrPost6, "did this work", "no");

    vector<Post*> testVec;
    testVec = drew1->increasingPriorityOldestToNewest();
    cout << "Testing update post stuff" << endl;
    for(int i = 0; i<testVec.size(); i++) {
        cout << testVec[i]->getPriority() << " and i index: " << i << " and " << testVec[i]->getUsername() << endl;
    }
    ...
    // for(int i = 0; i<drew1->low_priority_.size(); i++) {
    //     cout << drew1->low_priority_[i]->getUsername() << endl;
    // }
    // cout << "calling remove function: " << endl;

    // drew1->removePost(ptrPost2);
